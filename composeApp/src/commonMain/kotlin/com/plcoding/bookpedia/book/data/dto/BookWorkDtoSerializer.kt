...
object BookWorkDtoSerializer : KSerializer<BookWorkDto> {
    override val descriptor: SerialDescriptor = buildClassSerialDescriptor("BookWorkDto") {
    ...
    override fun deserialize(decoder: Decoder): BookWorkDto = decoder.decodeStructure(descriptor) {
        ...
        while (true) {
            when (val index = decodeElementIndex(descriptor)) {
                0 -> {
                    val jsonDecode = decoder as? JsonDecoder ?: throw SerializationException("This class can be loaded only by Json")
                    val element = jsonDecode.decodeJsonElement()
